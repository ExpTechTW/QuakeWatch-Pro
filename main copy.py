...
def clean_old_data():
    ...
    cutoff_time = current_time - DATA_WINDOW_LENGTH
...
def parsing_thread(ser):
    """獨立的資料解析線程"""
    global first_timestamp
    from datetime import datetime

    print(f"[解析線程] 已啟動 (時間窗口: {DATA_WINDOW_LENGTH} 秒)\n")
    report_interval = 1.0  # 每秒輸出一次統計
    clean_interval = 2.0   # 每 2 秒清理一次舊資料
    filter_interval = 0.5  # 每 0.5 秒應用一次濾波
    ...
    while parsing_active.is_set():
        ...
        if current_check_time - parse_stats['last_report_time'] >= report_interval:
            with data_lock:
                ...
                # print(
                #     f"[緩衝區] 已解析: {parse_stats['total_parsed']} 筆 | 速率: {rate:.1f} 筆/秒 | 緩衝: {len(x_data)} 個感測器 ({time_span:.1f}秒), {len(intensity_history)} 個強度")
                parse_stats['last_report_count'] = parse_stats['total_parsed']
                parse_stats['last_report_time'] = current_check_time
...
def update_plot(frame):
    ...
    with data_lock:
        # 計算 X 軸範圍
        x_min = current_time - DATA_WINDOW_LENGTH if current_time > DATA_WINDOW_LENGTH else 0
        x_max = current_time if current_time > DATA_WINDOW_LENGTH else max(
            DATA_WINDOW_LENGTH, current_time + 1)
...
def main():
    ...
    # 啟動獨立的解析線程
    parser = threading.Thread(target=parsing_thread, args=(ser,), daemon=True)
    parser.start()
