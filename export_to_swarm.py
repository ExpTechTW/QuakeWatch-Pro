...
DURATION = None       # 持續時間（秒），None = 匯出所有資料
# =================================
...
def get_output_prefix():
    if DURATION is not None:
        return f'swarm_{START_TIME}_{START_TIME+DURATION}s'
    else:
        return f'swarm_{START_TIME}_all'
...
def export_to_miniseed(sensor_rows, output_file='seismic_data.mseed'):
    ...
    # 三個分量
    components = [
        ('ENE', x_data),  # E-W (East-West) - X 軸
        ('ENN', y_data),  # N-S (North-South) - Y 軸
        ('ENZ', z_data)   # Vertical - Z 軸
    ]
    ...
    print(f"  包含 3 個 Trace (ENE, ENN, ENZ)")
...
def main():
    ...
    if DURATION is not None:
        ...
        print(f"時間範圍: {START_TIME} 秒 ~ 全部資料")
    print("="*60)
    ...
    if DURATION is not None:
        ...
        mseed_filename = f'seismic_data_{START_TIME}_all.mseed'
